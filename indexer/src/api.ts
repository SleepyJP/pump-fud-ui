...
import { FEES, GRADUATION_LIQUIDITY, TREASURY_WALLET } from './config';
...
app.get('/api/leaderboard/airdrop', (req, res) => {
  try {
    ...
    const leaderboard = getAirdropLeaderboard(limit);
    ...
    res.json({
      ...
      feeStructure: {
        buyTotalBps: FEES.BUY_TOTAL_BPS,
        buyUserBps: FEES.BUY_USER_BPS,
        buyTreasuryBps: FEES.BUY_TREASURY_BPS,
        sellTotalBps: FEES.SELL_TOTAL_BPS,
        sellUserBps: FEES.SELL_USER_BPS,
        sellTreasuryBps: FEES.SELL_TREASURY_BPS,
      },
    });
  } catch (error) {
...
app.get('/api/leaderboard/roi', (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 100, 500);
    const leaderboard = getROILeaderboard(limit);
...
app.get('/api/user/:address/rank', (req, res) => {
  try {
    ...
    const stats = getUserStats(address);
    const totalPool = getTotalPoolContribution();
...
app.post('/api/referral/register', (req, res) => {
  try {
    ...
    const referrerAddress = findReferrerByCode(referralCode);
    if (!referrerAddress) {
    ...
    // Record the referral
    const success = recordReferral(referrerAddress, referredAddress);
    if (!success) {
...
app.get('/api/referral/code/:code', (req, res) => {
  try {
    ...
    const referrerAddress = findReferrerByCode(code);

    if (!referrerAddress) {
      return res.status(404).json({ error: 'Invalid referral code' });
    }

    const stats = getUserStats(referrerAddress);
...
app.get('/api/fees', (req, res) => {
  res.json({
    ...
    fees: {
      buy: {
        totalBps: FEES.BUY_TOTAL_BPS,
        userPoolBps: FEES.BUY_USER_BPS,
        treasuryBps: FEES.BUY_TREASURY_BPS,
      },
      sell: {
        totalBps: FEES.SELL_TOTAL_BPS,
        userPoolBps: FEES.SELL_USER_BPS,
        treasuryBps: FEES.SELL_TREASURY_BPS,
      },
      graduation: {
        treasuryFeeBps: FEES.GRADUATION_FEE_BPS,
      },
      referral: {
        bps: FEES.REFERRAL_BPS,
      },
    },
    graduationLiquidity: {
      pulsexV2Percent: GRADUATION_LIQUIDITY.PULSEX_V2_PERCENT,
      paisleyV2Percent: GRADUATION_LIQUIDITY.PAISLEY_V2_PERCENT,
      lpBurned: true,
    },
  });
});
